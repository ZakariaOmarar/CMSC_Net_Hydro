...
def fig29_worked_example() -> None:
    ...
    err_srp = np.linalg.norm(srp_est - gt)
    err_tdoa = np.linalg.norm(tdoa_est - gt)
    fig.suptitle(
        f"Worked example on one D5 knock burst: SRP error {err_srp:.3f} m, "
        f"accel-TDOA error {err_tdoa:.3f} m",
        fontsize=9.5, y=1.02,
    )
    fig.tight_layout()
