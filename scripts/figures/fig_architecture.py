"""Architecture diagrams for Chapter 4 (figures 10-16 of the figure plan).
...
def fig10_system() -> None:
    ...
    box(ax, 7.0, 13.2, 8.2, 4.4,
        "2D CNN +\nattentive stats pool\n$\\to$ token per mic", fc=AC_BG, ec=ACOUSTIC, fs=7.2)
    box(ax, 7.0, 3.8, 8.2, 4.4,
        "1D CNN +\nattentive stats pool\n$\\to$ token per accel.", fc=VIB_BG, ec=VIBRATION, fs=7.2)
    ...
    box(ax, 16.6, 7.2, 8.4, 7.4,
        ...
        fc=CTX_BG, ec=INTERMEDIATE, fs=7.2)
    ...
    ax.text(27.6, 8.4, "context vector\n(operating state)", ha="center", va="top",
            fontsize=6.8, color=INTERMEDIATE)

    # V3: flow on top, threshold below, alert diamond to the right
    box(ax, 27.0, 14.4, 8.6, 4.0,
        "conditional flow\n6 coupling layers\nFiLM($c_t$) $\\Rightarrow$ exact NLL",
        fc="#fbe9e9", ec=ANOMALY, fs=7.2)
    box(ax, 29.8, 3.6, 8.0, 3.6, "per-cluster threshold\n95th pct, K-means($c_t$)",
        fc=GRAY_BG, ec="0.4", fs=7.0)
    ...
    ax.text(37.8, 14.9, "NLL", fontsize=7.0, color=ANOMALY, ha="left")
    arrow(ax, (36.2, 5.4), (38.3, 9.1), connectionstyle="arc3,rad=-0.2", color="0.4", lw=1.3)
